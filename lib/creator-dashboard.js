...
function createProjectRows(submissions, projectEvents) {
  const groupedByProject = new Map();

  projectEvents.forEach((event) => {
    const projectKey = event.item_title || event.item_id || "Unbekannt";
    const current = groupedByProject.get(projectKey) || {
      detailViews: 0,
      introOpens: 0,
      externalClicks: 0
    };
    ...
    if (event.event_type === "intro_open") current.introOpens += 1;
    if (event.event_type === "external_click") current.externalClicks += 1;

    groupedByProject.set(projectKey, current);
  });

  return submissions.map((submission) => {
    const counters =
      groupedByProject.get(submission.project_name) ||
      groupedByProject.get(buildSubmissionSlug(submission)) || {
        detailViews: 0,
        introOpens: 0,
        externalClicks: 0
      };
    ...
    return {
      ...
      detailViews: counters.detailViews,
      introOpens: counters.introOpens,
      externalClicks: counters.externalClicks
    };
  });
}
...
export async function fetchCreatorDashboardData(sessionEmail, includeAdminQueue) {
  ...
  return {
    ...
    ownProjectEvents,
    ownMagicLinkEvents,
    ownProjectRows: createProjectRows(submissions, ownProjectEvents),
    queue
  };
}
